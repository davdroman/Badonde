...
final class JSONFileInteractorSpy: JSONFileInteractor {
	typealias WriteSpy = (_ rawObject: [String: Any], _ path: String) -> Void
...
final class ConfigurationTests: XCTestCase {
	...
	func testInit_withEmptyFile() throws {
		let config = try Configuration(contentsOfFile: Fixture.empty.path, supportedKeyPaths: [])
		XCTAssertTrue(config.rawObject.isEmpty)
	}

	func testInit_withEmptyArrayFile() throws {
		let config = try Configuration(contentsOfFile: Fixture.emptyArray.path, supportedKeyPaths: [])
		XCTAssertTrue(config.rawObject.isEmpty)
	}

	func testInit_withEmptyDictionaryFile() throws {
		let config = try Configuration(contentsOfFile: Fixture.emptyDictionary.path, supportedKeyPaths: [])
		XCTAssertTrue(config.rawObject.isEmpty)
	}

	func testInit_withArrayFile() throws {
		let config = try Configuration(contentsOfFile: Fixture.array.path, supportedKeyPaths: [])
		XCTAssertTrue(config.rawObject.isEmpty)
	}
}
...
extension ConfigurationTests {
	func testGetRawValue_ofStringValue_withPlainKey() throws {
		let config = try Configuration(contentsOfFile: Fixture.dictionary.path, supportedKeyPaths: [])
...
extension ConfigurationTests {
	func testSetValue_ofStringValue_withNewPlainKey() throws {
		...
		let config = try Configuration(contentsOfFile: fixture.path, supportedKeyPaths: [], fileInteractor: interactor)
